...
*/
  ...
  let myGuild = bot.guilds.get("719324250942472252");
  let memberCount = myGuild.memberCount;
  let memberCountChannel = myGuild.channels.get("719355444056817665");
  memberCountChannel.setName("👥 Üyeler : " + memberCount)
  .then(result => console.log(result))
  .catch(error => console.log(error));
  
  let guild = bot.guilds.get("719324250942472252");
  let online = guild.members.filter(m => !m.user.bot && m.user.presence.status !== "online").size;
  guild.channels.get("719355470011170898").setName("🟢 Çevrimiçi : " + online)
  //.then(result => console.log(result))
  .catch(error => console.log(error));
...
let guild = client.guilds.get(`Sunucu ID'ni yaz.`) // kanalın bulunduğu sunucu id
let online = guild.members.filter(m => !m.user.bot && m.user.presence.status !== "offline").size;
let onnl = `Toplam Üye: ${guild.members.size}\nAktif Üye: ${online}`

setInterval(() => {
client.channels.get(`Kanal ID'sini yaz.`).setTopic(`${onnl.replace(`1`, ` :one: `).replace(/2/, ` :two: `).replace(`3`, ` :three: `).replace(/4/, ` :four: `).replace(`5`, ` :five: `).replace(/6/, ` :six: `).replace(`7`, ` :seven: `).replace(/8/, ` :eight: `).replace(`9`, ` :nine: `).replace(/0/, ` :zero: `)}`) 
}, 3000);  })
